...
我们也可以将一个文件路径数组传递给 `entry` 属性，这将创建一个所谓的"multi-main entry"。在你想要一次注入多个依赖文件，并且将它们的依赖关系在一个"chunk"中时，这种方式就很有用。
...
- `runtime`：运行时 chunk 的名字。如果设置了，就会创建要给新的运行时 chunk。在 webpack 5.43.0 之后将其设为 `false` 以避免一个新的运行时 chunk。
