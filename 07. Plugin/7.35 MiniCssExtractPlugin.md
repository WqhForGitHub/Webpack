...
>⚠️ source map 只在 `source-map`/`nosources-source-map`/`hidden-nosources-source-map`/`hidden-source-map` 值情况下起作用，因为 CSS 仅支持带有 `sourceMappingURL` 注释的 source map (例如 `//# sourceMappingURL=style.css.map`)。如果你需要将 `devtool` 设置为其他值，你可以使用 `css-loader` 中的 [`sourceMap: true`](https://github.com/webpack-contrib/css-loader#sourcemap) 来启用提取并生成 CSS 的 source map。
...
| `insert`                      | `{String, Function}` | `var head = document.getElements[0]`<br>`head.appendChild(linkTag)` | 在指定位置将 [非初始（由 async 修饰）](https://webpack.docschina.org/concepts/under-the-hood/#chunks) 的 CSS chunk 插入 `link` 标签    |
...
| `linkType`                    | `{String, Booolean}` | `text/css`                                                          | 允许使用自定义 link 类型加载异步 chunk                                                                                           |
...
### chunkFilename
类型：`String, Function` 默认值：`base on filename`
...
移除 Order 警告具体细节请参阅示例。
### insert
类型：`String, Function` 默认值：`document.head.appendChild(linkTag);`
>⚠️ 仅对 [非初始（由 async 修饰）](https://webpack.docschina.org/concepts/under-the-hood/#chunks) chunk 有效。
...
### runtime
类型：`Boolean` 默认值：`true`
允许开启/禁用 runtime 生成。 CSS 仍将被提取，并可用于自定义加载方法。 例如，你可以使用 [assets-webpack-plugin](https://github.com/ztoben/assets-webpack-plugin) 来检索它们，然后当需要时使用你自己的 runtime 代码下载静态资源。
...
如果没有显式启用，则默认启用（即可以通过 `true` 与 `false` 显式控制该配置），并且新 API 处于可用状态（webpack 版本至少为 `5.52.0`）。 布尔值在 `5.33.2` 版本后可用，但是你需要启用 `experiments.executeModule`（webpack `5.52.0` 版本不需要）。
...
| 名称           | 类型                   | 默认值                                |
| ------------ | -------------------- | ---------------------------------- |
| `publicPath` | `{String, Function}` | `webpackOptions.output.publicPath` |
| `emit`       | `{Boolean}`          | `true`                             |
...
```
## Named export for CSS Modules[](https://webpack.docschina.org/plugins/mini-css-extract-plugin/#name-export-for-css-modules)
