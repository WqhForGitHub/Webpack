...
# 关于 source map 的说明
**只对 [`devtool`](https://webpack.docschina.org/configuration/devtool/) 选项的 `source-map`，`inline-source-map`，`hidden-source-map` 和 `nosources-source-map` 有效**。
...
允许你自定义压缩函数。 默认情况下，插件使用 [terser](https://github.com/terser/terser) 库。 对于使用和测试未发布的版本或 fork 的代码很帮助。
...
```
### terserOptions
类型： `Object` 默认值： [默认](https://github.com/terser/terser#minify-options)
...
```
`Function<(node, comment) -> Boolean>`[](https://webpack.docschina.org/plugins/terser-webpack-plugin/#functionnode-comment-----boolean)
...
```
### [`uglify-js`](https://github.com/mishoo/UglifyJS)[](https://webpack.docschina.org/plugins/terser-webpack-plugin/#uglify-js)
...
```
### [`swc`](https://github.com/swc-project/swc)[](https://webpack.docschina.org/plugins/terser-webpack-plugin/#swc)
...
```
### [`esbuild`](https://github.com/evanw/esbuild)[](https://webpack.docschina.org/plugins/terser-webpack-plugin/#esbuild)
...
```
### Typescript[](https://webpack.docschina.org/plugins/terser-webpack-plugin/#typescript)
