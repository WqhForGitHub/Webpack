...
>提示
>[createapp.dev](https://createapp.dev/webpack) 是一个创建自定义 webpack 配置的在线工具。它允许你选择各种特性，其将会被添加到最后的配置文件中。它也会基于生成的 webpack 配置初始化一个示例项目，你可以在浏览器中查看该项目并且下载。

通常你的项目还需要继续扩展此能力，为此你可以在项目根目录下创建一个 `webpack.config.js` 文件，然后 webpack 会自动使用它。
下面指定了所有可用的配置选项。
>提示
>刚开始学习 webpack？请查看我们提供的指南，从 webpack 一些核心概念开始学习吧。
